...
function form_select($records,$args) {
	...
	##			 'style' => 'CSS Style',						// Extra Styles for Text Box
	##			 'extra' => 'Extra Code, Javascript, etc',		// For additional JS, to Disable Field, or any additional options can be entered here.
	##			 'filter' = > 'Show All',						// First option listed, usually this is used for filters
	##			));
	####################################################################
	...
	if(is_array($args)) { 
		...
		if(isset($args['display'])) { 
			...
			if(($value == '' && isset($args['required'])) || !isset($args['required'])) {
				if(isset($args['filter']) && $args['filter'] != '') {
					$tmp .= "<option value=''>".$args['filter']."</option>";	
				} else {
			...
			if(!is_array($records)) {
				while($row = mysqli_fetch_assoc($records)) {
					...
					if(isset($row['chrKEY'])) {
						$tmp .= "<option".($value != '' && $row['chrKEY'] == $value ? ' selected="selected"' : '')." value='".$row['chrKEY']."'>".$row['chrRecord']."</option>";
					} else {
